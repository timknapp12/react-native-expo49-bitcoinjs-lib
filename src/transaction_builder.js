...
async function expandInput(scriptSig, witnessStack) {
  var prevOutScript;
  var prevOutType;
  var scriptType;
  var script;
  ...
  if (sigType === scriptTypes.P2SH) {
    ...
    prevOutScript = bscript.scriptHash.output.encode(
      await bcrypto.hash160(redeemScript)
    );
  ...
  if (classifyWitness === scriptTypes.P2WSH) {
    ...
    if (scriptSig.length === 0) {
      prevOutScript = bscript.witnessScriptHash.output.encode(
        await bcrypto.sha256(witnessScript)
      );
      ...
      witnessProgram = bscript.witnessScriptHash.output.encode(
        await bcrypto.sha256(witnessScript)
      );
    ...
    var keyHash = await bcrypto.hash160(key);
    if (scriptSig.length === 0) {
...
async function expandOutput(script, scriptType, ourPubKey) {
  ...
  switch (scriptType) {
    ...
    case scriptTypes.P2PKH:
      ...
      var pkh2 = await bcrypto.hash160(ourPubKey);
    ...
    case scriptTypes.P2WPKH:
      ...
      var wpkh2 = await bcrypto.hash160(ourPubKey);
...
async function prepareInput(
  ...
  if (redeemScript && witnessScript) {
    redeemScriptHash = await bcrypto.hash160(redeemScript);
    witnessScriptHash = await bcrypto.sha256(witnessScript);
    checkP2shInput(input, redeemScriptHash);

    if (
      !redeemScript.equals(
        bscript.witnessScriptHash.output.encode(witnessScriptHash)
      )
    )
    ...
    prevOutScript = bscript.pubKeyHash.output.encode(
      await bcrypto.hash160(kpPubKey)
    );
  ...
  if (signType === scriptTypes.P2WPKH) {
    signScript = bscript.pubKeyHash.output.encode(
      bscript.witnessPubKeyHash.output.decode(signScript)
    );
  }
...
function buildStack(type, signatures, pubKeys, allowIncomplete) {
  if (type === scriptTypes.P2PKH) {
    if (
      signatures.length === 1 &&
      signatures[0] instanceof Buffer &&
      pubKeys.length === 1
    )
      return bscript.pubKeyHash.input.encodeStack(signatures[0], pubKeys[0]);
  } else if (type === scriptTypes.P2PK) {
    if (signatures.length === 1 && signatures[0] instanceof Buffer)
    ...
    if (signatures.length > 0) {
      ...
      return bscript.multisig.input.encodeStack(
        signatures /* see if it's necessary first */
      );
    }
  } else {
...
TransactionBuilder.prototype.__addInputUnsafe = function (
  ...
  if (!input.prevOutScript && options.prevOutScript) {
    ...
    input.prevOutType =
      prevOutType || bscript.classifyOutput(options.prevOutScript);
  }
...
TransactionBuilder.prototype.__build = function (allowIncomplete) {
  ...
  this.inputs.forEach(function (input, i) {
    ...
    if (!allowIncomplete) {
      if (
        SIGNABLE.indexOf(result.type) === -1 &&
        result.type !== bscript.types.P2WPKH
      ) {
